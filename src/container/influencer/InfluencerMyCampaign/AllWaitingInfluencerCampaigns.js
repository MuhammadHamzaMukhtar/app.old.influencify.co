...
class AllWaitingInfluencerCampaigns extends Component {
	constructor(props) {
		super(props);
		this.state = {
			openDialog: false,
	...
	onLoadMore = () => {
		if (this.state.limit < (this.props.waitingAllInfluencerCampaigns || []).length) {
			this.setState({
		...
		return (
			<div>
				<div className="grid grid-cols-12 gap-5">
					{(this.props.waitingAllInfluencerCampaigns || []).length > 0 ? (
						this.props.waitingAllInfluencerCampaigns
							.slice(0, this.state.limit)
							.map((waitingAllCampaign, index) => (
								<div
									className="md:col-span-4 sm:col-span-6 col-span-12"
									key={index}
								>
									{/* <span className="text-right block">
										{waitingAllCampaign.campaignDate.formatedPostingFrom} -{" "}
										{waitingAllCampaign.campaignDate.formatedPostingTo}
									</span> */}
									<div className="shadow-[0px_4px_5px_#96969640] hover:shadow-[0px_10px_30px_#96969640] bg-white rounded-[8px] p-0">
										<Link to={`/influencer/influencer-booking/${waitingAllCampaign.id}`}>
										<div className="block">
											<div
												className="flex items-center mb-4 py-4 pl-[1rem]"
												onClick={() =>
													this.handleClickOpen(
														waitingAllCampaign.campaignUserEmail
													)
												}
											>
												<div>
													<img
														src={
															waitingAllCampaign.campaignUserAvatar
																? waitingAllCampaign.campaignUserAvatar
																: avatar
														}
														alt={waitingAllCampaign.campaignUserName}
														className="w-[52px] rounded-full"
													/>
												</div>
												<div className="pl-3">
													<h4 className="font-semibold text-[20px]">
														{waitingAllCampaign.campaignUserName}
													</h4>
												</div>
											</div>
										</div>
										...
										>
											<div className="flex flex-col px-4">
												<div className="py-12 flex justify-between">
													<p>Campaign</p>
														...
														{waitingAllCampaign.campaignTitle}
													</b>
												</div>
											</div>
											<div className="h-[1px] w-full bg-[#0000001f]" />
											<p className="py-12 px-4 flex items-center">
											{waitingAllCampaign?.platform?.name=="instagram" &&

											<FaInstagram className="success mr-3" />

											}
											{waitingAllCampaign?.platform?.name=="youtube" &&
											<FaYoutube className="success mr-3" />

											}
											{waitingAllCampaign?.platform?.name=="tiktok" &&
											<FaTiktok className="success mr-3" />

											}
											{waitingAllCampaign?.platform?.name}
											</p>
											<div className="h-[1px] w-full bg-[#0000001f]" />
											<div className="flex flex-col px-4">
												<div className="py-12 flex justify-between">
													...
													<h6 className=" text-[16px]">
														{waitingAllCampaign.fixedFee}{" "}
														{this.props.currentLoggedUser.currency_code}{" "}
														{waitingAllCampaign.products &&
														waitingAllCampaign.products.length
																? "+ Voucher"
															: ""}
													</h6>
												</div>
											...
											<div className="flex flex-col px-4">
												<div className="py-12 flex justify-between">
													...
													<h6 className=" text-[16px]">
													<span className="text-right block">
														{waitingAllCampaign.campaignDate.formatedPostingFrom} - {waitingAllCampaign.campaignDate.formatedPostingTo}
													</span>
													</h6>
												</div>
											</div>
				...
				{this.state.limit < (this.props.waitingAllInfluencerCampaigns || []).length ? (
