...
class RejectedClosedInfluencerCampaigns extends Component {
	constructor(props) {
		super(props);
		this.state = {
			openDialog: false,
	...
	onLoadMore = () => {
		if (this.state.limit < (this.props.rejectedInfluencerCampaigns || []).length) {
			this.setState({
		...
		return (
			<div>
				<div className="grid grid-cols-12 gap-5">
					{(this.props.rejectedInfluencerCampaigns || []).length > 0 ? (
						this.props.rejectedInfluencerCampaigns
							...
							.map((closedCampaign, index) => (
								...
								>
									<span className="text-right block">
										{closedCampaign.campaignDate.postingFrom} -{" "}
										{closedCampaign.campaignDate.postingTo}
									</span>
				...
				{this.state.limit < (this.props.rejectedInfluencerCampaigns || []).length ? (
