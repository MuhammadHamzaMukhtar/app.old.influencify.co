...
class CanceledInfluencerCampaigns extends Component {
	constructor(props) {
		super(props);
		this.state = {
			openDialog: false,
	...
	onLoadMore = () => {
		if (this.state.limit < (this.props.canceledInfluencerCampaigns || []).length) {
			this.setState({
		...
		return (
			<div>
				<div className="grid grid-cols-12 gap-5">
					{(this.props.canceledInfluencerCampaigns || []).length > 0 ? (
						this.props.canceledInfluencerCampaigns
							...
							.map((closedCampaign, index) => (
								...
								>
									...
									<div className="shadow-[0px_4px_5px_#96969640] hover:shadow-[0px_10px_30px_#96969640] bg-white rounded-[8px] p-0">
										<Link to={`/influencer/influencer-booking/${closedCampaign.id}`} >
											...
											<div className="flex flex-col px-4">
												<div className="py-12 flex justify-between">
													...
													<span className="text-right block">
														{closedCampaign.campaignDate.formatedPostingFrom} -{" "}
														{closedCampaign.campaignDate.formatedPostingTo}
													</span>
												</div>
											</div>
										</div>
				...
				{this.state.limit < (this.props.canceledInfluencerCampaigns || []).length ? (
