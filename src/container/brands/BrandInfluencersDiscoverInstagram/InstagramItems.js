...
const InstagramItems = (props) => {
  ...
  const onPageChanged = (pageData) => {
    if (props.refreshData.is_admin) {
      const payload = Object.assign({}, props.payload);
      const form = Object.assign({}, props.form);
      payload["paging"]["skip"] = (pageData?.currentPage || 0) * (pageData?.pageLimit || 0) - pageData?.pageLimit;
      form["loadMore"] = true;
      props.searchFilters(payload, form);
      let query = {
        platform: props.platform,
  ...
  const campaignWithSelected = (type) => {
    const filter = props.payload.filter.with_contact?.find((i) => i.type == 'email');
        ...
        <div className="lg:w-9/12 md:w-8/12 w-full order-2 md:!order-1 pr-3">
          ...
          <div className="inline-flex items-center justify-center">
            {!isLoading && (influencerTotal || 0) > 12 &&
              <Pagination
                totalRecords={Math.min((influencerTotal || 0), 10000) - 12}
                pageLimit={12}
                pageNeighbours={12}
              ...
              <>
                ...
                >
                  ...
                  <div className="w-full">
                    <Button
                      className="px-4 rounded-[8px] h-[40px] text-[14px] inline-flex justify-center border-[1px] items-center bg-white border-[#7c3292] hover:opacity-80 w-full black font-medium"
                    ...
                    {toggleButton && (
                      <>
                        {props.campaign_types &&
                          props.campaign_types.map((item, key) => (
                            <Button
                              key={key}
                              onClick={() =>
                ...
                <div className="text-center mb-4 shadow-[0px_4px_5px_#96969640] hover:shadow-[0px_10px_30px_#96969640] bg-white rounded-[8px] p-4  font-medium">
                  <div className="w-full">
                    <Button
                      onClick={showInfluencerList}
