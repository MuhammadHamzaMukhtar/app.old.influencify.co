	...
	render() {
		...
		return (
			<Popover className="flex xxs:w-auto w-full items-center relative">
				<Popover.Button
					className={`${
						payload.FromComments || payload.ToComments
								...
								<div className="w-full">
									...
									<div className="sm:flex items-center">
										<div className="mb-4 sm:!mb-0">
											<input
												value={payload.FromComments || ""}
												onChange={(e) =>
													this.addFilterPayload("FromComments", e.target.value)
												}
										...
										<div className="ml-0 sm:!ml-4">
											<input
												value={payload.ToComments || ""}
												className="rounded-[8px] h-[40px] inline-flex w-full items-center px-3 border-[1px] border-[#ced4da] focus-visible:outline-0 focus:border-[#7c3292]"
												onChange={(e) =>
													this.addFilterPayload("ToComments", e.target.value)
												}
												placeholder="To"
											/>
										</div>
