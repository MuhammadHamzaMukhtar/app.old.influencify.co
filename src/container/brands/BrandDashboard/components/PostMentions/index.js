...
class PostMentions extends Component {
	constructor(props) {
		super(props);
		this.timeout = 0;
		this.state = {
	...
	async componentDidMount() {
		const json = await Influencify.businessDiscoveryMedia();
		if (json !== undefined) {
			if (json.status === 200) {
	...
	render() {
		...
		return (
			<>
				...
				<div className="infl-slick-carousel post-mention">
											...
											>
												{item.media_type === "VIDEO" ? (
													<div>
														<video
															controls
															className="w-full rounded-[8px] min-h-[190px] object-contain"
															src={item.media_url}
														></video>
													</div>
												) : (
