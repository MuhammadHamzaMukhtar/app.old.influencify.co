...
class Mention extends Component {
	...
	render() {
		const { payload, realMentions } = this.props;
		const { query_mention } = this.state;
		let filteredTags =
			query_mention === ""
				? realMentions
				: realMentions.filter((tag) =>
						tag.text
							.toLowerCase()
							.replace(/\s+/g, "")
							.includes(query_mention.toLowerCase().replace(/\s+/g, ""))
				  );
										...
										<div className="flex items-center">
											<div className="w-full">
												<Combobox
													...
													onChange={(data) => this.addPayload("mention", data)}
												>
														...
														<div className="relative w-full cursor-default flex items-center overflow-hidden border border-[#22242626] rounded-lg bg-white text-left focus-visible:outline-0 sm:text-sm">
															<Combobox.Button className="w-full">
																<Combobox.Input
																	...
																	displayValue={payload.mention}
																	placeholder="Select Mention"
																	onChange={(e) =>
																		this.setState({
																			query_mention: e.target.value,
																		})
																	}
																/>
															</Combobox.Button>
...
const mapStateToProps = ({ MonitoringCampaign }) => {
	return {
		...
		realMentions: MonitoringCampaign.real_mentions,
	};
};
