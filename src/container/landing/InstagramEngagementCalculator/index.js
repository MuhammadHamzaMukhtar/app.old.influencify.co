...
class InstagramEngagementCalculator extends Component {
	constructor(props) {
		super(props);
		this.state = {
			profileModal:false
	...
	search = () => {
		...
		const payload = {
			...
			filter: {
				username: {
					value: this.props.form.instagramEngagementText,
					operator: "exact",
				},
			},
			paging: {
		...
		if ((this.props.form.instagramEngagementText || "").length > 1) {
	...
	render() {
		...
		const title = helper.instagram_engagement_calculator_title;
		const description = helper.instagram_engagement_calculator_description;
					...
					<div className="containers">
						<div className="mt-12 text-center">
							...
							<div className="flex items-center justify-center">
								<div className="lg:w-6/12 w-full my-4 flex">
									<input
										placeholder="Type your account name..."
										className="border-[1px] border-[#ced4da] xs:text-[14px] text-[10px] min-w-unset w-full focus:border-[#7c3292] h-14 px-[1rem] focus-visible:outline-0 rounded-l-[8px]"
										type="text"
										onChange={(e) => {
											
											if (count <= process.env.REACT_APP_MARKETING_TOOL_LOCK) {
												this.addForm("instagramEngagementText", e.target.value);
											}
										}}
										readOnly={count > process.env.REACT_APP_MARKETING_TOOL_LOCK}
										disabled={count > process.env.REACT_APP_MARKETING_TOOL_LOCK}
									/>
									<Button
										disabled={count > process.env.REACT_APP_MARKETING_TOOL_LOCK}
										onClick={this.search}
...
const mapStateToProps = ({ marketingTool, influencerSearch }) => {
	return {
		data: marketingTool.instaEngagementData,
		count: marketingTool.instaEngagementCount,
		loader: marketingTool.instaEngagementLoader,
...
const mergeProps = (stateProps, dispatchProps, ownProps) => {
	...
	return {
		...
		search: (data) => {
			return actions.fetchInstagramEngagementCalculator(dispatch, data);
		},
		addForm: (data) => {
