...
class YoutubeNiche extends Component {
	constructor(props) {
		super(props);
		this.state = {
			showSection: false,
	...
	search = () => {
		const payload = {
			...
			filter: {
				...
				text: this.props.form.searchYoutubeInfluencersText,
			},
			paging: {
				skip: 0,
				limit: 5,
		...
		const data = { payload, platform };
		 if ((this.props.form.searchYoutubeInfluencersText || "").length > 1) {
	...
	render() {
		...
		const title = helper.youtube_niche_title;
		const description = helper.youtube_niche_description;
					...
					<div className="containers">
						<div className="mt-12 text-center">
							...
							<h1 className="black sm:text-[35px] text-[30px] text-center mt-6 mb-6 font-semibold">
								...
								Find Youtube Influencers in Your Niche
							</h1>
									...
									<div className="rounded-[8px] shadow-[0px_4px_5px_#96969640] hover:shadow-[0px_10px_30px_#96969640] transition-all duration-300 bg-white border-[#dedede] p-12">
										<h5 className="text-[18px] black font-medium text-center mb-12">
											...
											<span className="font-normal inline-block black">
												were found in {form.searchYoutubeInfluencersText}
											</span>
										</h5>
...
const mapStateToProps = ({ marketingTool, influencerSearch }) => {
	return {
		data: marketingTool.searchYoutubeInfluencers,
		total: marketingTool.searchYoutubeTotalInfluencers,
		count: marketingTool.searchYoutubeCountInfluencers,
		loader: marketingTool.searchYoutubeInfluencerLoader,
...
const mergeProps = (stateProps, dispatchProps, ownProps) => {
	...
	return {
		...
		search: (data) => {
			return actions.fetchSearchYoutubeInfluencers(dispatch, data);
		},
		addForm: (data) => {
...
export default connect(mapStateToProps, undefined, mergeProps)(YoutubeNiche);
