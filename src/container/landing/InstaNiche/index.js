...
class InstaNiche extends Component {
	constructor(props) {
		super(props);
		this.state = {
			showSection: false,
	...
	search = () => {
		const payload = {
			...
			filter: {
				...
				text: this.props.form.searchInstagramInfluencersText,
			},
			paging: {
				skip: 0,
				limit: 5,
		...
		const data = { payload, platform };
		 if ((this.props.form.searchInstagramInfluencersText || "").length > 1) {
	...
	render() {
		...
		const title = helper.instagram_niche_title;
		const description = helper.instagram_niche_description;
								...
								<div className="lg:w-6/12 sm:w-full my-4">
									<div className="w-full flex">
										<input
											...
											className="border-[1px] min-w-unset w-full border-[#ced4da] focus:border-[#7c3292] h-14 px-[1rem] focus-visible:outline-0 rounded-l-[8px]"
											type="text"
											onChange={(e) => {
												if (
													...
													this.addForm(
														"searchInstagramInfluencersText",
														e.target.value
													);
												}
											}}
										...
										<Button
											...
											onClick={this.search}
											className="xxs:px-12 px-6 text-white xxs:text-[14px] text-[12px]  h-14 bg--purple rounded-r-[8px] inline-flex items-center text-[14px] font-semibold"
											text="Check"
										/>
									</div>

									<div className="flex flex-wrap items-center mt-4">
										<p className="darkGray mr-4 py-[0.5rem] mb-4">
											Key word e.g.{" "}
										</p>
										<p className="success mr-4 bg--lightSuccess rounded-[8px] py-[0.5rem] px-4 mb-4">
											Crypto{" "}
										</p>
										<p className="success mr-4 bg--lightSuccess rounded-[8px] py-[0.5rem] px-4 mb-4">
											Fashion{" "}
									...
									<div className="rounded-[8px] shadow-[0px_4px_5px_#96969640] hover:shadow-[0px_10px_30px_#96969640] transition-all duration-300 bg-white border-[#dedede] p-12">
										<h5 className="text-[18px] black font-medium text-center mb-12">
											...
											<span className="font-normal inline-block black">
												were found in {form.searchInstagramInfluencersText}
											</span>
										</h5>
										...
										<div className="text-center">
											...
											<LinkTo
												to="/brand/register"
												text="Get Started"
												className="text-white bg--purple rounded-[8px] h-[40px] text-[14px] inline-flex items-center justify-center hover:opacity-80 px-12"
											/>
										</div>
										}
									</div>
...
const mapStateToProps = ({ marketingTool, influencerSearch }) => {
	return {
		data: marketingTool.searchInstagramInfluencers,
		total: marketingTool.searchInstagramTotalInfluencers,
		count: marketingTool.searchInstagramCountInfluencers,
		loader: marketingTool.searchInstagramInfluencerLoader,
...
const mergeProps = (stateProps, dispatchProps, ownProps) => {
	...
	return {
		...
		search: (data) => {
			return actions.fetchSearchInstagramInfluencers(dispatch, data);
		},
		addForm: (data) => {
...
export default connect(mapStateToProps, undefined, mergeProps)(InstaNiche);
