...
import { FaSpinner, FaTiktok } from "react-icons/fa";
...
export default function TiktokMiniProfileInfo(props) {
					...
					<div className="items-center flex flex-col h-full justify-center">
						...
						<div className="w-full mb-6">
							<div className="flex items-center justify-center mt-4 mb-1">
								<FaTiktok size={22} className="darkGray" />
								<h3 className="ml-2 text-[28px] text-black font-medium">
									{account?.user_profile?.url
										? <a target={"_blank"} href={account?.user_profile?.url}>{account?.user_profile?.username}</a>
										: ""}
								</h3>
							</div>
