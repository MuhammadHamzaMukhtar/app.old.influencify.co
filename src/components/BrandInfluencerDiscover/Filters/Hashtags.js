	...
	render() {
		...
		return (
			<Popover className="flex items-center relative">
				<Popover.Button
					className={`${(form.filter.text_tags || []).filter((i, index) => i.type == 'hashtag')?.length > 0
								...
								<div className="w-full">
									<div className="mb-1 font-medium whitespace-nowrap flex items-center justify-between">
										<div className="flex items-center">
											...
											<Tooltip
												...
												tooltipText={tooltip.hashtags_tooltip}
												placement="top-left"
											/>
										</div>
									</div>
									<div className="flex items-center border rounded-[8px]">
										<input
											...
											onChange={(e) => this.handleSearchFilters(e, "hashtag")}
											onKeyDown={(e) => this._handleKeyDown(e, 'text_tags', 'hashtag')}
											onFocus={() => this._onFocus('is_hashtags_focused')}
											onBlur={() => this._onBlur('is_hashtags_focused')}
											value={this.state.hashtag}
										/>
										{this.state.is_hashtags_focused === false ? (
											<div className="mx-2">
									...
									<>
										{form.filter.text_tags && form.filter.text_tags.map((i, index) => i.type == 'hashtag' &&
...
export default connect(mapStateToProps, undefined, mergeProps)(Hashtags);
